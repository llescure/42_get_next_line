...
/*   Updated: 2021/01/09 14:44:50 by llescure         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	char				*buf;
	static char			*temp[FD_SIZE];
	int					char_read;

	char_read = 1;
	if (error_case(fd, line, &temp[fd], &buf) < 0)
		return (-1);
	if (!(ft_strchr(temp[fd], '\n')))
	{
		...
		{
			buf[char_read] = '\0';
			temp[fd] = copy_to_join(temp[fd], buf);
			if (ft_strchr(buf, '\n'))
				break ;
		}
	}
	...
	*line = ft_trim(temp[fd], '\n');
	temp[fd] = copy_to_get_temp(temp[fd]);
	return (return_value(char_read, &temp[fd]));
}
